...
def _process_data(intensity_data):
    ...
    batch_index = 0
    y_index = 0
    for foldername in tqdm(('yes', 'no', 'up', 'down', 'left', 'right', 'on', 'off', 'stop', 'go')):
        loc = base_loc / foldername
        for filename in os.listdir(loc):
            # fix to support more recent torchaudio version
            add = str(loc / filename)
            if version.parse(torchaudio.__version__) < version.parse("0.9.0"):

                audio, _ = torchaudio.load_wav(add, channels_first=False,
                                               normalization=False)
            else:

                audio, _ = torchaudio.load(add, channels_first=False,
                                           normalize=False)
